...
proc newline {}  {
global   menu  new	  valeur final testligne fichier   lignemax	ligne i  lmax longm Psource  testres  equi	Piso Yiso Ziso Fcor Demi qmax  imprtemps Qtotal  tab
...
global   menu  new	  valeur final testligne fichier   lignemax	ligne i  lmax longm Psource  testres  equi	Piso Yiso Ziso Fcor Demi qmax  imprtemps Qtotal  tab
	...
	#gets $io v ; append imprtemps $v 
	}
		 
	}

	puts " Psource $Psource Qtotal $Qtotal imprtemps $imprtemps"
	close $io
	return 
}
...
global   menu  new	  valeur final testligne fichier   lignemax	ligne i  lmax longm Psource  testres  equi	Piso Yiso Ziso Fcor Demi qmax  imprtemps Qtotal  tab
	...
	if {$max < $Q }  {
		button $final.erreur -text "DN trop petit ou Longueur trop importante ou Pression du noeud amont trop faible\n   noeud [expr $i + 1] "  -command {
			destroy $final
			pack $fichier $fichier.file
			pack $menu $menu.b1 $menu.b2  $menu.b3 -padx 5 -pady 5
			}
	...
	pack $final.valid
	puts "Qtotal $Qtotal "	
		if {$Qtotal  >   [ expr $debit  *  1.05 ] }	 {
			label $final.alert -text "Le débit total est différent de 5% du débit total préconisé"
			pack $final.alert
			puts Qtotal 0.0
		}
		if {$Qtotal  <   [ expr  $debit  *  0.95 ] }	 {
			label $final.alert -text "Le débit total est différent de 5% du débit total préconisé"
			pack $final.alert	
			puts Qtotal 0.0
		}
...
global   menu  new	  valeur final testligne fichier   lignemax	ligne i  lmax longm Psource  testres  equi	Piso Yiso Ziso Fcor Demi qmax  imprtemps Qtotal  tab
	...
	pack .impr
	pack forget $menu $fichier
	set filename "impression"
	button .impr.retour -text "Retour au programme " \
		-command {
		destroy .impr
		pack $fichier $fichier.file
		pack $menu $menu.b1 $menu.b2  $menu.b3 -padx 5 -pady 5
...
#	set v "\n"
	...
	puts $io "\n"
	puts $io "\n"
	puts $io "Chubb Securite Marine Nantes"
	puts $io "\n"
	...
	puts "Qtotal $Qtotal"
		...
		if {$Qtotal  <=  [ expr $debit  *  1.1 ] }	 {
			if {$Qtotal  >=  [ expr  $debit  *  0.9  ] }	 {
				puts $io $imprtemps
				puts $io "\n"
			}
		}
...
proc auteur {} {
global   menu  new	  valeur final testligne fichier   lignemax	ligne i  lmax longm Psource  testres  equi	Piso Yiso Ziso Fcor Demi qmax  imprtemps tab

	frame .aut
	pack .aut
	pack forget $menu $fichier
	
	button .aut.retour -text "Auteur : Pierre Lecanu \n Version 1.0 du 16 Mai 2004  pour Chubb Marine \n Retour au programme" \
		-command {
		destroy .aut
		pack $fichier $fichier.file
		pack $menu $menu.b1 $menu.b2  $menu.b3 -padx 5 -pady 5
...
radiobutton .ini.taux1  -text "Taux 0.75 kg par litre  (température maxi. 40°)" -variable taux -value [expr 1.0 /  0.75] -command { message ; .ini.txt configure -text $msg  }
radiobutton .ini.taux2 -text "Taux 0.68 kg par litre (température maxi. 49°)" -variable taux -value [expr 1.0  /  0.68] -command   { message ; .ini.txt configure -text $msg }
radiobutton .ini.taux3 -text "Taux 0.55 kg par litre (température maxi. 65°)" -variable taux -value [expr 1.0  / 0.55] -command  { message ; .ini.txt configure -text $msg}
radiobutton .ini.vol1 -text "Volume de la bouteille 67  litres" -variable vol -value 67.0 -command  { message ;  .ini.txt configure -text $msg  }
radiobutton .ini.vol2 -text "Volume de la bouteille 60  litres" -variable vol  -value 60.0 -command  { message ;  .ini.txt configure -text $msg  }
radiobutton .ini.vol3 -text "Volume de la bouteille 40  litres" -variable vol  -value 40.0 -command  {  message ; .ini.txt configure -text $msg }
radiobutton .ini.vol4 -text "Volume de la bouteille 27  litres" -variable vol -value 27.0 -command  {  message ; .ini.txt configure -text $msg  }
radiobutton .ini.vol5 -text "Volume de la bouteille 20  litres" -variable vol -value 20.0 -command  {	 message ; .ini.txt configure -text $msg }
radiobutton .ini.vol6 -text "Volume de la bouteille 13.4  litres" -variable vol  -value 13.4 -command  { mesage ; .ini.txt configure -text $msg  }
radiobutton .ini.vol7 -text "Volume de la bouteille 8  litres" -variable vol  -value 8.0   -command  { mesage ;   .ini.txt configure -text $msg  }
scale .ini.nb -label "Nombre de bouteilles" -variable nb  -resolution 1 -from 0 -to 100 \
	-orient horizontal   -length 300 -showvalue true	 -command  {  message ; .ini.txt configure -text $msg  }
scale .ini.tp -label "Donner le temps de diffusion théorique" \
	-resolution 0.01 -from 0.01  -to 15.0  -orient horizontal \
	-variable tptheo   -length 300  -showvalue true  -command  { message ;  .ini.txt configure -text $msg }
scale .ini.pourc -label "Donner le pourcentage temporelle "  -resolution 0.1 -from 0.1 -to 100.0 -orient  horizontal\
	-variable pour	-length 300  -showvalue true  -command  { message ;  .ini.txt configure -text $msg } 
	
button .ini.retour -text "Retour au menu avec sauvegarde"	 -command {
		destroy .ini
		pack $fichier $fichier.file
		pack $menu $menu.b1 $menu.b2  $menu.b3 -padx 5 -pady 5
		set imprtemps  "le taux de chargemnt de CO2 par bouteille est de [format %.2f $taux ] en kg/litre\n"
		append imprtemps "le volume d'une bouteille de CO2 est de [format %.2f $vol ]  en litres\n"
		append imprtemps "Le nombre de bouteilles est de [format %3.0f $nb ]\n"
		append imprtemps "Le temps de diffusion theorique est de [format %.2f $nb ]\n"
		append imprtemps "Le pourcentage temporelle est [format %.0f $pour ] %\n"
		append imprtemps "Le debit de diffusion de CO2 est de  [format %.2f [expr $nb * ( $vol / $taux ) / ( $pour  / 100.0 * $tptheo)] ] en kg/min\n"
		append imprtemps "La quantite totale de CO2 emise est de  [format %.2f [expr $nb * ($vol / $taux) ] ] en kg\n"
		append imprtemps "\n"
		append imprtemps "Le temps de diffusion est de   [format %.2f  [expr $pour / 100.0 * $tptheo] ] en minute\n"
		set Qtotal   [expr $nb * ( $vol / $taux ) / ( $pour  / 100.0 * $tptheo)]
		puts "imprtemps $imprtemps"
		return
		}
button .ini.annul -text "Retour au menu"	 -command {
		destroy .ini
		pack $fichier $fichier.file
		pack $menu $menu.b1 $menu.b2  $menu.b3 -padx 5 -pady 5
		return
...
set tptheo 0.00001
set pour 85.0
set msg  "Le temps de diffusion imposée en minute\n"
append msg  "le débit de diffusion de CO2 est en kg/min\n"
append msg  "La quantité totale de CO2 émise est en kg"
label .ini.txt -text $msg



pack .ini.taux1 .ini.taux2 .ini.taux3 .ini.vol1 .ini.vol2 .ini.vol3 .ini.vol4 .ini.vol5 .ini.vol6 .ini.vol7 .ini.nb .ini.tp .ini.pourc
pack	.ini.txt .ini.retour  .ini.annul
...
set msg  "Le temps de diffusion imposée est de   [format %.2f  [expr $pour / 100.0 * $tptheo] ] en minute\n"
append msg  "le débit de diffusion de CO2 est de  [format %.2f [expr $nb * ( $vol / $taux ) / ( $pour  / 100.0 * $tptheo)] ] en kg/min\n"
append msg  "La quantité totale de CO2 émise est de  [format %.2f [expr $nb * ($vol / $taux) ] ] en kg"
.ini.txt configure -text $msg
...
$fichier.file.choix add separator
$fichier.file.choix add command -label "Outil - temps et quantité émise préconisés" -command {pack forget $fichier   $menu; tempsquant}
$fichier.file.choix add separator
